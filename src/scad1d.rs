//! Tool for creating 1D objects.
//!
//! These can be exported as SVG, or given a width to become SCAD 2D objects.
...
impl SVG for D1 {
    fn svg(&self) -> String {
        match &self {
            D1::Line(XY(x1,y1), XY(x2,y2)) => format!(r#"<line x1="{}" y1="{}" x2="{}" y2="{}" stroke="goldenrod" stroke-linecap="round" />"#, x1, y1, x2, y2) +"\n",
...
impl D1 {
    pub fn line<IXY0: Into<XY>, IXY1: Into<XY>>(ixy0: IXY0, ixy1: IXY1) -> D1 {
        D1::Line(ixy0.into(), ixy1.into())
    }

    pub fn to_svg(&self, prop: SvgProp) -> String {
        let w = prop.width;
        let h = prop.height;
