...
impl D2 {
    ...
    pub fn translate_vec<XY: Into<Real2> + Clone>(&self, xy: XY, n: u32) -> Vec<D2> {
        (0..n).map(move |ii| self.translate(v2(xy.clone().into().0.x * ii as f32, xy.clone().into().0.y * ii as f32))).collect::<Vec<_>>()
    }
