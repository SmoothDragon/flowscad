...
fn main() {
    ...
    let center_column = D3::cylinder(h_center, d_center/2.)
        ...
        .add(D3::cuboid(h_notch, w_notch, h_outer).translate(d_outer/2.-0.5, -w_notch/2., 0.))
        .difference(D3::cylinder(h_center+2., d_center/2.-2.).translate(0.,0.,-1.))
        .difference(D3::cuboid(2.*d_center, 14., 3.7*2.).translate(-d_center, -7., h_center-3.7))
        ;
