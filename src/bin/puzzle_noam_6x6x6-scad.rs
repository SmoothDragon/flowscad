...
fn main() {
    ...
    let gap = 0.2;
    let piece = vec![
    D3::beveled_cube_block( (4, 4, 4), l_cube, bevel, gap)
        ,
    D3::beveled_cube_block( (4, 4, 2), l_cube, bevel, gap)
        .translate(v3(5.0*l_cube,0.,0.))
        ,
    D3::beveled_cube_block( (2, 5, 5), l_cube, bevel, gap)
        .add(D3::beveled_cube_block( (3, 1, 5), l_cube, bevel, gap))
        .translate(v3(0., 5.0*l_cube, 0.))
        ,
    D3::beveled_cube_block( (2, 5, 1), l_cube, bevel, gap)
        .add(D3::beveled_cube_block( (3, 1, 1), l_cube, bevel, gap))
        .translate(v3(4.*l_cube, 5.0*l_cube, 0.))
        ,
    D3::beveled_cube_block( (3, 5, 1), l_cube, bevel, gap)
        .translate(v3(8.*l_cube, 5.0*l_cube, 0.))
        ,
    D3::beveled_cube_block( (3, 4, 1), l_cube, bevel, gap)
        .translate(v3(10.*l_cube, 0., 0.))
        ,
    D3::beveled_cube_block( (2, 2, 3), l_cube, bevel, gap)
        .add(D3::beveled_cube_block( (3, 1, 3), l_cube, bevel, gap))
        .translate(v3(0.*l_cube, 11.0*l_cube, 0.))
        ,
    D3::beveled_cube_block( (1, 2, 3), l_cube, bevel, gap)
        .add(D3::beveled_cube_block( (3, 1, 3), l_cube, bevel, gap))
    ...
    for (ii, item) in piece.iter().enumerate() {
        fs::write(format!("puzzle_6x6x6_{ii}.scad"), format!("{}", item)).expect("Unable to write file");
    }
}
