...
fn pips(num: u32, d_pip:f32, ir_square:f32) -> D3 {
    ...
    match num {
        ...
        3 => [(s,s,0.),(s,s,1.), (s,s,2.)].into_iter().map(|(x,y,i)| ball.clone().translate((x,y,0.0)).rotate((0,0,i*120.))).union(),
        4 => [(0.,0.,0.),(s,s,0.),(s,s,1.), (s,s,2.)].into_iter().map(|(x,y,i)| ball.clone().translate((x,y,0.0)).rotate((0,0,i*120.))).union(),
        // 5 => ball,
        // 6 => [(s,s),(-s,-s)].into_iter().map(|(x,y)| ball.clone().translate((x,y,0.0))).union(),
        // 7 => [(s,s,0.),(s,s,1.), (s,s,2.)].into_iter().map(|(x,y,i)| ball.clone().translate((x,y,0.0)).rotate((0,0,i*120.))).union(),
...
fn main() -> Result<()> {
    ...
    let base = D3::octahedron(r_oct)
        ...
        .rotate_y(109.47/2.0)
        ;
    let side1 = pips(4, d_pip, s_pip)
        .translate_z(pip_shift)
        .add(pips(4, d_pip, s_pip)
            .translate_z(-pip_shift)
        )
    ...
    let side3 = pips(3, d_pip, s_pip)
        .translate_z(pip_shift)
        .add(pips(3, d_pip, s_pip)
            .translate_z(-pip_shift)
        )
        .rotate( (0, -180.0+109.47, 240) )
        .render()
        ;
    let side4 = pips(1, d_pip, s_pip)
        .translate_z(pip_shift)
        .add(pips(1, d_pip, s_pip)
            .translate_z(-pip_shift)
        )
        .render()
        ;
