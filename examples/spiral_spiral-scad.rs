...
fn main() -> Result<()> {
    ...
    let eye = D2::circle_r(r_eye).translate_x(r_eye/2)
        .intersection(D2::circle_r(r_eye).translate_x(-r_eye/2))
        ;

    let angle_inner = X(1.4);
    let angle_outer = X(1.);
    ...
    let outer = D3::cylinder_r(h-gap, r_eye) -
        (0..layers)
        .map(|ii| D2::circle_r(0.866*r_eye+gap)
            // .rotate(ii * angle_inner)
            .translate_y(0.5*r_eye)
            .rotate(ii * angle_outer)
            .linear_extrude(h_layer)
