...
fn main() -> Result<()> {
    ...
    let perimeter_lip = perimeter.clone()
        ...
        // .add(D2::rounded_rectangle((25,11), 2).center()
        .add(D2::rounded_rectangle((25,15), 2).center()
            .sub(D2::rounded_rectangle((14,8), 1).center())
            // .translate_y(box_l/2 + l/2 + 2.0)
            .translate_y(-40)
            .sub(holes.clone())
            // .add(holes.clone().translate((-box_w, 1.9*l)))
            // .add(holes.clone())
            .chamfer45_round(wall)
            .add_map(|x| x.mirror([0,0,1]))
        )
    ...
    let perimeter_outline = perimeter.clone()
        ...
        .sub(id_shape.clone().translate_y(-20))
    ...
    let perimeter_id = perimeter_outline.clone()
        ...
        // Move to top of badge
        // .sub(D3::cuboid((2*box_w, box_l, 2*box_h))
            // .rotate_x(45)
            // .translate((-box_w, 1.9*l,0))
        // )
        .translate_z(box_h+id_vert)
        // This is the nubbin that holds the top of the badge in.
        // Probably shold be replaced by curved berm
